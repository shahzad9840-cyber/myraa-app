    ...
    if "class MainActivity" in content and "onCreate" not in content:
        content = content.replace(
            ...
            (
                ...
                "    public void onPause() {\n"
                "        super.onPause();\n"
                "        // Android auto-pauses WebView JS timers when the app is\n"
                "        // backgrounded to save battery. That silently breaks the\n"
                "        // screen-share polling loop (and could stall voice audio),\n"
                "        // so force JS timers to keep running while minimized.\n"
                ...
                "    }\n"
                "\n"
                "    private void acquireAudioSubsystem() {\n"
                "        // Fixes the common Android WebView bug where getUserMedia audio\n"
                "        // fails with \"NotReadableError: Could not start audio source\"\n"
                "        // even though the mic permission is granted - the audio hardware\n"
                "        // needs to be explicitly acquired via AudioManager first.\n"
