 ...
 *
 * pingInterval is 60s. A shorter interval (15s) was tried first but
 * caused the opposite problem on this network: round-trips that
 * simply take a bit longer than 15s were being misread as a dead
 * connection, triggering a reconnect (which starts a BRAND NEW Gemini
 * session) in the middle of a perfectly fine conversation - felt like
 * audio delay, mid-sentence cutouts, and MYRAA "forgetting" what she
 * was doing. 60s gives slow-but-alive connections enough room, while
 * the reconnect logic below still recovers automatically if the
