...
public class MyraaNativePlugin extends Plugin {
    ...
    public void executeAction(PluginCall call) {
        // JS sends the whole action payload nested under an "action" key:
        // { action: { action: "FLASHLIGHT_ON", target, message, query,
        //             destination, title, timestamp, durationSeconds,
        //             volume, cameraCommand, confirmed, confirmationToken } }
        JSObject payload = call.getObject("action");
        if (payload == null) {
            JSObject ret = new JSObject();
            ret.put("ok", false);
            ret.put("message", "No action specified");
            call.resolve(ret);
            return;
        }
        ...
        if ("SEND_SMS".equals(action)) {
            if (getPermissionState("sms") != PermissionState.GRANTED) {
                pendingActionParams = payload;
                requestPermissionForAlias("sms", call, "smsPermissionCallback");
                return;
            }
            sendSmsDirect(call, payload);
            return;
        }
        if ("CALL_CONTACT".equals(action)) {
            ...
            placeCallDirect(call, payload);
            return;
        }

        try {
            JSONObject result = DeviceActionExecutor.execute(getActivity(), action, new JSONObject(payload.toString()));
    ...
    private void sendSmsDirect(PluginCall call, JSObject payload) {
        JSObject ret = new JSObject();
        try {
            String number = payload.getString("target");
    ...
    private void placeCallDirect(PluginCall call, JSObject payload) {
        JSObject ret = new JSObject();
        try {
            String number = payload.getString("target");
