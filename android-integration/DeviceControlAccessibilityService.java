...
public class DeviceControlAccessibilityService extends AccessibilityService {
    ...
    protected void onServiceConnected() {
        ...
        instance = this;
        // Gesture callbacks run on this dedicated background thread
        // instead of the main thread, so a caller that blocks waiting
        // for one (like findAndTap) can never deadlock against it.
     ...
     *  description best matches the given text, and taps its center.
    ...
    private AccessibilityNodeInfo findBestTextMatch(AccessibilityNodeInfo root, String needle) {
        ...
        while (!queue.isEmpty()) {
            ...
            if (!label.isEmpty() && (node.isClickable() || node.isCheckable())) {
                int score = -1;
    ...
    public void scrollScreen(String direction, android.graphics.Point screenSize, final BoolCallback callback) {
        ...
        switch (direction.toLowerCase()) {
            case "up": y1 = bottomY; y2 = topY; break;      // content moves up
            case "down": y1 = topY; y2 = bottomY; break;    // content moves down
