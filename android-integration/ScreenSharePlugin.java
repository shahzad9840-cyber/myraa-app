...
public class ScreenSharePlugin extends Plugin {
    ...
    private void handleScreenCaptureResult(PluginCall call, ActivityResult result) {
        ...
        // Capture at a smaller resolution - full screen resolution makes
        // every frame's JPEG encode/decode expensive and causes lag.
        // 720px on the long edge is plenty for AI vision / live preview.
        int realWidth = metrics.widthPixels;
        int realHeight = metrics.heightPixels;
        int maxDimension = 480;
        double scale = Math.min(1.0, (double) maxDimension / Math.max(realWidth, realHeight));
        int width = (int) Math.round(realWidth * scale);
        int height = (int) Math.round(realHeight * scale);
